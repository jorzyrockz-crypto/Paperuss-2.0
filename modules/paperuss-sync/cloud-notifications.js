...
// cross-device sign-in/sync. Left blank, PapeRuss runs fully offline ΓÇö
...
const UPLOAD_RETRY_BASE_MS=30000; // 30s base; doubles each failure: 30sΓåÆ1mΓåÆ2mΓåÆ4mΓåÆ8m
...
async function continueAsGuest(){
  ...
  toast('Continuing in guest mode ΓÇö everything stays on this device');
}
...
async function signInWithGoogle(fromLanding){
  if(!firebaseReady){
    toast('Cloud sign-in is not configured ΓÇö continuing offline');
...
async function signInWithEmailPassword(){
  ...
  setEmailAuthBusy(true); setEmailAuthMessage('Signing inΓÇª');
  try{
...
async function createEmailPasswordAccount(){
  ...
  setEmailAuthBusy(true); setEmailAuthMessage('Creating accountΓÇª');
  try{
    ...
    toast('Account created ΓÇö check your inbox to verify your email');
    await seedAndSyncAccount(user.uid);
  }catch(error){
    setEmailAuthMessage(authErrorMessage(error),true);
  }finally{
...
async function sendPasswordReset(){
  ...
  setEmailAuthBusy(true); setEmailAuthMessage('Sending reset linkΓÇª');
  try{
...
/* ============================================================
   OFFLINE UPLOAD QUEUE ΓÇö persists upload IDs across reloads
   ============================================================ */
...
/* Dynamic upload timeout ΓÇö 1 byte/ms minimum, floor at 20s, ceiling at 15min */
