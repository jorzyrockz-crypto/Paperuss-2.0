/* ============================================================
   PAPERUSS 2.0 ΓÇö BRANCH ENGINE (Sidebar Categories & Notebooks)
   ============================================================ */
...
(function () {
  ...
  function renderNoteBranchSelector(note, disabled = false) {
    ...
    branches.forEach(branch => {
      select.add(new Option(`${branch.parentId ? 'ΓÇö ' : ''}${branch.name}`, branch.id));
    });
    ...
    rootBranches.forEach(b => {
      ...
      html += `
        <div class="branch-item ${isSelected ? 'active' : ''}" draggable="true" data-branch-id="${b.id}" data-branch-name="${escHtml(b.name)}">
          ...
          <button type="button" class="branch-opt-btn" data-branch-id="${b.id}" title="Branch Options">Γï«</button>
        </div>
      `;
        ...
        subBranches.forEach(sub => {
          ...
          html += `
            <div class="branch-item sub-branch-item ${subSelected ? 'active' : ''}" draggable="true" data-branch-id="${sub.id}" data-branch-name="${escHtml(sub.name)}">
              ...
              <button type="button" class="branch-opt-btn" data-branch-id="${sub.id}" title="Branch Options">Γï«</button>
            </div>
          `;
        });
        html += `</div>`;
      ...
      modal.innerHTML = `
        <div class="branch-modal-card">
          <div class="print-modal-header">
            ...
            <button type="button" class="tool-btn" id="btnCloseBranchModal">Γ£ò</button>
          </div>
